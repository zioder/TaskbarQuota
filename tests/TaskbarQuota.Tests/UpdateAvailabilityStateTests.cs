...
public sealed class UpdateAvailabilityStateTests
{
    ...
    [Theory]
    [InlineData("9.9.9", "1.1.2", UpdateCheckResultKind.UpdateAvailable)]
    [InlineData("v9.9.9", "1.1.2", UpdateCheckResultKind.UpdateAvailable)]
    [InlineData("1.1.2", "1.1.2", UpdateCheckResultKind.UpToDate)]
    [InlineData("1.0.0", "1.1.2", UpdateCheckResultKind.UpToDate)]
    public void Fake_release_override_uses_version_ordering(
        string fakeVersion,
        string currentVersion,
        UpdateCheckResultKind expectedKind)
    {
    ...
    [Fact]
    public void Fake_release_override_rejects_non_numeric_versions()
    {
        Assert.Null(UpdateCheckerService.TryCreateFakeUpdate(
            "9.9.9-preview",
            "1.1.2",
            AppDistributionChannel.UnsignedGitHub));
    }
